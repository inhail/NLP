...
# state比input多一行的原因是:
# state[:,0] 是原始state，state[:,1]是state[:,0]經過計算後的值
# state[:,1] 是原始state，state[:,2]是state[:,1]經過計算後的值 ...以此類推
...
for iter in range(number_of_epoch):
    ...
    loss = np.square(state[:,3] - np.squeeze(y)).sum() / len(x)

    if iter % 1000 == 0:
        print("Current iter : ", iter, " Current loss: ", loss)
        print("state_1_out : ", state_1_out)
        print("state_2_out : ", state_2_out)
        print("state_3_in : ", state_3_in)
